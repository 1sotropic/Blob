...
while keep_going == 'yes' or keep_going == 'y' or keep_going == 'Y' or keep_going == 'Yes':
    while operation == 'z':
        ...
        if choice == 'a':
            ...
            operation = 'z'
    if choice != 'p':
        print ('How many numbers would you like to %s?' % (operation))
        numbers = int(raw_input('How many numbers would you like to %s? ' % (operation)))
        numbera = array('d', 0)
        #float numbera[0] = 0.0
        for umm in range (0, numbers):
            numbera[umm] = float(raw_input('Enter a number:'))
